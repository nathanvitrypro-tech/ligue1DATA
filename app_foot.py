...
try:
    ...
    st.error("⚠️ Clé API non trouvée ! Configurez les secrets.")
    st.stop()
...
def get_ligue1_data():
    """Récupère Classement + Prochains Matchs"""
    LEAGUE_ID = 61
    SEASON = 2025
    headers = get_headers()
    ...
    # 2. PROCHAINS MATCHS (FIXTURES) - Nouveau !
    url_fixtures = "https://api-football-v1.p.rapidapi.com/v3/fixtures"
    # On prend les 10 prochains matchs programmés
    ...
    if 'response' in data_fixtures:
        for f in data_fixtures['response']:
            # Formatage de la date (ex: 12/01 21:00)
    ...
    if 'response' in data:
        for p in data['response']:
            ...
            players.append({
                ...
                'Note': float(stats['games']['rating']) if stats['games']['rating'] else 0.0,
...
if not df_standings.empty:
    ...
    with tab1:
        ...
        c2.metric("Meilleure Attaque", df_standings.sort_values('BP', ascending=False).iloc[0]['Equipe'])
        c3.metric("Meilleure Défense", df_standings.sort_values('BC', ascending=True).iloc[0]['Equipe'])
        c4.metric("Matchs Joués", df_standings.iloc[0]['MJ'])
        
        st.divider()
        
        # Graphique Forme (Scatter amélioré)
        st.subheader("📊 Dynamique du Championnat")
        fig = px.scatter(df_standings, x="BC", y="BP", 
                         text="Equipe", size="Pts", color="Forme",
                         color_discrete_sequence=px.colors.qualitative.Bold,
                         title="Efficacité : Attaque (Y) vs Défense (X)")
        fig.update_xaxes(autorange="reversed", title="Buts Encaissés (Solidité)")
        fig.update_yaxes(title="Buts Marqués (Puissance)")
        st.plotly_chart(fig, use_container_width=True)
        
        # Tableau Classement Pro
        st.dataframe(
    ...
    with tab2:
        st.subheader("📅 Les 10 prochains matchs")
        if not df_fixtures.empty:
            for index, row in df_fixtures.iterrows():
                # Design "Billet de match"
                with st.container():
                    c_date, c_dom, c_vs, c_ext, c_stade = st.columns([2, 3, 1, 3, 3])
                    c_date.write(f"**{row['Date']}**")
                    
                    with c_dom:
                        st.image(row['Logo_Dom'], width=30)
                        st.write(row['Domicile'])
                    
                    c_vs.markdown("<h4 style='text-align: center; color: #888;'>VS</h4>", unsafe_allow_html=True)
                    
                    with c_ext:
                        st.image(row['Logo_Ext'], width=30)
                        st.write(row['Exterieur'])
    ...
    with tab3:
        ...
        selected_team = st.selectbox("Choisir une équipe :", df_standings['Equipe'].unique())
        ...
        if not df_players.empty:
            # 1. RADAR CHART (Comparaison avec la moyenne ligue)
            club_data = df_standings[df_standings['Equipe'] == selected_team].iloc[0]
            avg_bp = df_standings['BP'].mean()
            avg_bc = df_standings['BC'].mean()
            
            # Normalisation (Score sur 100 approximatif)
            offense_score = min(100, (club_data['BP'] / df_standings['BP'].max()) * 100)
            defense_score = min(100, (1 - (club_data['BC'] / df_standings['BC'].max())) * 100 + 20) # +20 bonus
            
            col_kpi, col_radar = st.columns([1, 2])
            
            with col_kpi:
                st.image(club_data['Logo'], width=100)
                st.metric("Points", club_data['Pts'])
                st.metric("Différence", f"{club_data['Diff']}")
                st.caption(f"Forme : {club_data['Forme']}")

            with col_radar:
                categories = ['Attaque', 'Défense', 'Points', 'Matchs Joués']
                # Valeurs arbitraires normalisées pour l'exemple visuel
                values = [offense_score, defense_score, club_data['Pts'], (club_data['MJ']/34)*100]
            ...
            # 2. TABLEAU JOUEURS ENRICHI
            st.write("#### 🏃 Effectif & Stats Détaillées")
            st.dataframe(
                df_players.sort_values(by='Min', ascending=False), # Tri par temps de jeu
                column_config={
            ...
            st.warning("Données joueurs indisponibles.")

else:
    st.error("Erreur de chargement. Vérifiez la clé API.")
